...
LottoTicket = mongoose.model('LottoTicketb', processedSchema);

var lottery_ticket_array = new mongoose.Schema({
  numbers: [Number],
});

LotteryNumberArray = mongoose.model('LotteryNumberArrayb', lottery_ticket_array);
...
LottoTicket2 = mongoose.model('LottoTicket2b', altSchema);
...
client.getBlockHash(294309, function(err, blockhash) {
...
LottoTicket.find({ amount: {$gte: .01}}).sort({timestamp: -1}).limit(20).exec(function(err, tickets){
...
var ticket_count = Math.floor(amount * 100);
